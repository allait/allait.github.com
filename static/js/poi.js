...
// Shamelessly borrowed from http://bottlepy.org and modified.

/*global jQuery $ document window */

var POI = function(elem, options) {
    ...
    this.elem = elem;
    this.anchor = this.elem.attr("id");
    this.title = this.elem.text();
    this.node  = $('<div>').addClass(this.css).appendTo('body');
    this.link  = $('<a>').text(this.title)
                         .attr('href', '#'+this.anchor)
                         .appendTo(this.node);
    ...
    // Smoothly scroll to element instead of jumping to anchor
    this.node.click(function() { POI.scrollto(self.elem); });
    this.link.click(function(e) {e.preventDefault();});
};
...
POI.show = function() {
    // Show all at once
    if(POI.hide_timeout) {
        window.clearTimeout(POI.hide_timeout);
    }
    POI.hide_timeout = null;
    jQuery.each(POI.all, function() {
        this.show();
...
POI.whereami = function() {
    ...
    var last = null;
    jQuery.each(POI.all, function() {
        if(position < this.elem.offset().top) {
          return false;
        }
        last = this;
    });
